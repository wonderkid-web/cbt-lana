...
// 1. Tambah Konsentrasi
if (isset($_POST['tambah'])) {
    $id_jurusan = $_POST['id_jurusan'];
    $nama = mysqli_real_escape_string($conn, $_POST['nama_konsentrasi']);
    $kode = strtoupper(mysqli_real_escape_string($conn, $_POST['kode']));
    mysqli_query($conn, "INSERT INTO konsentrasi (id_jurusan, nama_konsentrasi, kode) VALUES ('$id_jurusan', '$nama', '$kode')");
    header("Location: konsentrasi.php?msg=disimpan");
    exit;
}

// 2. Edit Konsentrasi
if (isset($_POST['edit_konsentrasi'])) {
    $id = $_POST['id_konsentrasi'];
    $id_jurusan = $_POST['id_jurusan'];
    $nama = mysqli_real_escape_string($conn, $_POST['nama_konsentrasi']);
    $kode = strtoupper(mysqli_real_escape_string($conn, $_POST['kode']));
    mysqli_query($conn, "UPDATE konsentrasi SET id_jurusan = '$id_jurusan', nama_konsentrasi = '$nama', kode = '$kode' WHERE id_konsentrasi = '$id'");
    header("Location: konsentrasi.php?msg=updated");
    exit;
}

// 3. Hapus Konsentrasi
if (isset($_GET['hapus']) && isset($_GET['id_konsentrasi'])) {
    $id = $_GET['id_konsentrasi'];
    mysqli_query($conn, "DELETE FROM konsentrasi WHERE id_konsentrasi = '$id'");
    header("Location: konsentrasi.php?msg=dihapus");
    exit;
}
...
// --- LOGIKA PENCARIAN ---
 ...
 $where_clause = "";
if (!empty($search)) {
    $where_clause = " WHERE (k.nama_konsentrasi LIKE '%$search%' OR k.kode LIKE '%$search%' OR j.nama_program LIKE '%$search%') ";
}
...
// --- LOGIKA PAGINATION ---
 ...
 $query_hitung = mysqli_query($conn, "SELECT COUNT(*) as total FROM konsentrasi k JOIN jurusan j ON k.id_jurusan = j.id_jurusan $where_clause");
 $res_hitung = mysqli_fetch_assoc($query_hitung);
 $total_data = $res_hitung['total'];
 $total_halaman = ceil($total_data / $batas);

 $query_data = mysqli_query($conn, "SELECT k.*, j.nama_program FROM konsentrasi k JOIN jurusan j ON k.id_jurusan = j.id_jurusan $where_clause ORDER BY j.nama_program ASC, k.nama_konsentrasi ASC LIMIT $halaman_awal, $batas");
...
<head>
    ...
    <title>Data Konsentrasi | SMK Putra Anda</title>
...
<div class="content-area" id="contentArea">
    ...
    <div class="container-fluid p-4">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4 gap-3">
            <div>
                <h4 class="fw-bold mb-0">Data Konsentrasi</h4>
                <p class="text-muted small mb-0">Kelola peminatan khusus pada setiap jurusan</p>
            </div>
            ...
            <form method="GET" action="" class="row g-2">
                <div class="col-md-10">
                    <div class="input-group">
                        ...
                        <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Cari konsentrasi atau jurusan..." value="<?= htmlspecialchars($search) ?>">
                    </div>
                </div>
                <div class="col-md-2">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light text-secondary">
                        <tr>
                            ...
                            <th class="px-4 py-3 border-0">Nama Konsentrasi</th>
                    ...
                    <tbody>
                        ...
                        <?php if(mysqli_num_rows($query_data) > 0): ?>
                            ...
                            <tr>
                                <td class="px-4"><?= $no++; ?></td>
                                <td class="px-4 fw-bold text-dark"><?= $d['nama_konsentrasi']; ?></td>
                                <td><span class="badge bg-primary bg-opacity-10 text-primary px-3"><?= $d['nama_program']; ?></span></td>
                                ...
                                <td class="text-center">
                                    <button class="btn btn-sm btn-outline-info btn-action me-1" 
                                            ...
                                            data-id="<?= $d['id_konsentrasi']; ?>" data-nama="<?= $d['nama_konsentrasi']; ?>" data-kode="<?= $d['kode']; ?>" data-jurusan="<?= $d['id_jurusan']; ?>">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <a href="konsentrasi.php?hapus=1&id_konsentrasi=<?= $d['id_konsentrasi']; ?><?= !empty($search) ? '&search='.$search : '' ?>" 
                                       class="btn btn-sm btn-outline-danger" onclick="return confirmHapus('<?= htmlspecialchars(addslashes($d['nama_konsentrasi'])); ?>', this.href)">
        ...
        <div class="modal-content border-0 shadow">
            <form action="" method="POST">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Tambah Konsentrasi Baru</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">JURUSAN</label>
                        <select name="id_jurusan" class="form-select" required>
                            ...
                            <?php if(mysqli_num_rows($query_jurusan) > 0): ?>
                                <?php while($j = mysqli_fetch_assoc($query_jurusan)): ?>
                                    <option value="<?= $j['id_jurusan']; ?>"><?= $j['nama_program']; ?> (<?= $j['kode']; ?>)</option>
                                <?php endwhile; ?>
                            <?php else: ?>
                    ...
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">NAMA KONSENTRASI</label>
                        <input type="text" name="nama_konsentrasi" class="form-control" placeholder="Contoh: Pemrograman Web dan Perangkat Bergerak" required>
                    </div>
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">KODE KONSENTRASI</label>
                        <input type="text" name="kode" class="form-control text-uppercase" placeholder="Contoh: PWEB" required>
                    </div>
                </div>
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <form action="" method="POST">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="bi bi-pencil-square me-2"></i>Edit Data Konsentrasi</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <input type="hidden" name="id_konsentrasi" id="edit_id">
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">JURUSAN</label>
                        <select name="id_jurusan" id="edit_jurusan" class="form-select" required>
                            <option value="">-- Pilih Jurusan --</option>
                            <?php 
                            // Reset pointer agar dropdown terisi ulang untuk modal edit
                    ...
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">NAMA KONSENTRASI</label>
                        <input type="text" name="nama_konsentrasi" id="edit_nama" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label class="small fw-bold text-muted mb-1">KODE KONSENTRASI</label>
                ...
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" name="edit_konsentrasi" class="btn btn-primary rounded-pill px-4">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
